...
verus! {
    ...
    // Problem 3
    spec fn spec_median_three(a:int, b:int, c:int) -> int {
        if b <= a <= c {
            a 
        } else if a <= b <= c {
            b
    ...
    spec fn spec_median_five(a:int, b:int, c:int, d:int, e:int) -> int {
    ...
    proof fn median_of_medians_filter(a:i32, b:i32, c:i32, d:i32, e:i32) -> (median: int) 
        requires
            a <= b <= c <= d <= e,
        ensures
    ...
    fn main() {
        ...
        // Problem 2
        let diff1 = absolute_difference(10, 5);
        let diff2 = absolute_difference(5, 10);
        assert(diff1 == diff2)
    }
